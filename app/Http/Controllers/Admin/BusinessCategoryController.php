...
Class BusinessCategoryController extends Controller
{
	...
	public function index(Request $request){
        ...
        return view('admin.businesscategory.index',$data);
    }
...
public function add(Request $request){
        ...
        if(is_numeric($id) && $id > 0){
            $categories = BusinessCategory::find($id);
            if(empty($categories)){
                return redirect($this->ADMIN_ROUTE_NAME.'/businesscategory');
            }
        }
        ...
        if($request->method() == 'POST' || $request->method() == 'post'){
            ...
            if(empty($back_url)){
                $back_url = $this->ADMIN_ROUTE_NAME.'/businesscategory';
            }
        ...
        $page_heading = 'Add Business Category';

        if(isset($categories->name)){
            $categories_name = $categories->name;
            $page_heading = 'Update Business Category - '.$categories_name;
        }  
        ...
        return view('admin.businesscategory.form', $data);
    ...
    public function save(Request $request, $id=0){
        ...
        $testimonial = new BusinessCategory;

        if(is_numeric($id) && $id > 0){
            $exist = BusinessCategory::find($id);
    ...
    private function saveImage($request, $testimonial, $oldImg=''){
        ...
        if ($file) {
            ...
            $thumb_path = 'businesscategory/thumb/';
    ...
    public function delete(Request $request){
        ...
        if(is_numeric($id) && $id > 0){
            $is_delete = BusinessCategory::where('id', $id)->delete();
        }
    ...
    public function ajax_delete_image(Request $request){
        ...
        if (is_numeric($id) && $id > 0) {
            $testimonial = BusinessCategory::find($id);
